...
impl create_normal_user::repository::Trait for Repository {
    async fn create_normal_user(
        &self,
        username: &str,
        password_hash: &str,
    ) -> Result<tml_domain::model::auth::user::Model, create_normal_user::repository::Error> {
        ...
        let new_user = match user_to_create.insert(&self.db).await {
            ...
            Err(e) => match e.sql_err() {
                Some(SqlErr::UniqueConstraintViolation(_)) => {
                    return Err(create_normal_user::repository::Error::UsernameDuplication);
                }
                _ => {
                    return Err(create_normal_user::repository::Error::Unknown(
                        e.to_string(),
                    ));
                }
            },
        ...
        let normal_user_role = role::Entity::find_by_name("normal-user")
            .one(&self.db)
            .await
            .map_err(|e| -> create_normal_user::repository::Error {
                create_normal_user::repository::Error::Unknown(e.to_string())
            })?
            .ok_or(create_normal_user::repository::Error::Unknown(
